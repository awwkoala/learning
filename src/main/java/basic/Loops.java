...
public class Loops {
  ...
  public String printIntsUntil(int x) {
    if (x <= 0) {
      throw new IllegalArgumentException("Method only works for positive numbers.");
    }
    String printInts = "";
    for (int i = 0; i <= x; i++) {
      printInts += i + " ";
...
//    Na przykład "pies" przekształci w "seip"
  ...
  public String intToBinary(int a) {
    ...
    return turnedAroundString(binary);
  }
